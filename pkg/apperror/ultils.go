...
func WriteError(c fiber.Ctx, httpStatus int, code string, message string) error {
...
func BadRequest(c fiber.Ctx, code status.ErrorCode, message string) error {
	error_code := fmt.Sprintf("AI-%d", code)
	return WriteError(c, fiber.StatusBadRequest, error_code, message)
}
...
func InternalError(c fiber.Ctx, err error) error {
	...
	return WriteError(c, fiber.StatusInternalServerError, error_code, err.Error())
}

// Success writes a standardized JSON success response
func Success(fiberCtx fiber.Ctx, response FiberSuccessMessage) error {
